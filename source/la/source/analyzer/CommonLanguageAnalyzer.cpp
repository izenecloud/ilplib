...
int CommonLanguageAnalyzer<LanguageAction, BasicSentence>::analyze(
        IDManager* idm, const Term & input, TermIdList & output,
        unsigned char retFlag)
{
    ...
    int localOffset = 0;
        ...
        input.text_.convertString(encode_, ustring_convert_buffer1, 4096*4);

        if( !analyzeSecond )
            return;

        BasicSentence* pE = lat_->getBasicSentence(ustring_convert_buffer1);
        ...
        for ( int i = 0; i < listSize; ++i )
        {
            ...
            for ( int j = 0; j < count; j++ )
            {
                ...
                size_t len2 = sizeof(UString::CharT) * UString::toUcs2(encode_, lexicon, len, (UString::CharT*)ustring_convert_buffer, 4096/sizeof(UString::CharT));
...
//                        prime_it->morpheme_ = morpheme;
                    ...
                    else
                    {
                        ...
                        idm->getTermIdByTermString(ustring_convert_buffer, len2, output.back().termid_);
