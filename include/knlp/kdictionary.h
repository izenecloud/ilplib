 ...
 * =====================================================================================
 */
#ifndef _ILPLIB_NLP_KDICTIONARY_H_
#define _ILPLIB_NLP_KDICTIONARY_H_
...
public:
    ...
    void fmm(std::string line, std::vector<std::pair<std::string, T> >& r, bool nor = true)const
    {
        ...
        izenelib::util::KString kstr(line);
    ...
    void
    subtokens(const std::vector<std::pair<std::string, T> >& tks,
      std::vector<std::pair<std::string, T> >& subs)const
    {
        subs.clear();
        for (uint32_t i=0; i<tks.size(); ++i)
        {
            izenelib::util::KString kstr(tks[i].first);
            if (kstr.length()<=2)
            {
